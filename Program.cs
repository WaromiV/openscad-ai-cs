...
app.MapPost("/mcp", async (HttpRequest request) =>
{
    ...
    using (body)
    {
        ...
        if (method == "tools/call")
        {
            ...
            if (!paramsElement.TryGetProperty("arguments", out var argsElement) || argsElement.ValueKind != JsonValueKind.Object)
            {
...
void BakeTopLabelAndLegend(string imagePath, string labelText)
{
    ...
    image.Mutate(ctx =>
    {
        foreach (var (idx, entry) in legend.Select((value, idx) => (idx, value)))
        {
            ...
            {
                ctx.DrawText(entry.Label, legendFont, Color.Black, new PointF(x0 + lineLen + 11, y - (legendFont.Size * 0.45f) + 1));
                ctx.DrawText(entry.Label, legendFont, entry.Color, new PointF(x0 + lineLen + 10, y - (legendFont.Size * 0.45f)));
            }
        }
    });
